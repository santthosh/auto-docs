...
namespace Microsoft.ServiceModel.Web
{
    ...
    class WebErrorHandler : IErrorHandler
    {
        ...
        public void ProvideFault(Exception error, MessageVersion version, ref Message fault)
        {
            ...
            if (webError == null)
            {
                if (error is SecurityAccessDeniedException) webError = new WebProtocolException(HttpStatusCode.Unauthorized, error.Message, error);
                else if (error is ServerTooBusyException) webError = new WebProtocolException(HttpStatusCode.ServiceUnavailable, error.Message, error);
                ...
                {
                    webError = new WebProtocolException(HttpStatusCode.InternalServerError, error.Message, error);
                }
            }
            if (version == MessageVersion.None)
            {
                WebMessageFormat format = (WebMessageFormat)OperationContext.Current.IncomingMessageProperties[ResponseWebFormatPropertyAttacher.PropertyName];
