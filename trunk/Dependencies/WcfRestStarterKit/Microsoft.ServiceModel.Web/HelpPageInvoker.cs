...
namespace Microsoft.ServiceModel.Web
{
    class HelpPageInvoker : IOperationInvoker
    {
        ...
        private string GetMethod(WebGetAttribute get, WebInvokeAttribute invoke)
        {
            ...
            if (!string.IsNullOrEmpty(invoke.Method)) return invoke.Method;
            return "POST";
        ...
        Message CreateExample(object instance, Type body, bool isXmlSerializerType)
        {
            if (isXmlSerializerType)
            {
        ...
        public Message GetRequestExample(string operation)
        {
            foreach (OperationDescription od in this.Description.Operations)
            {
                if (od.Name == operation)
                {
                    ...
                    try
                    {
                        instance = Activator.CreateInstance(body);
                    }
                    catch (Exception e)
                    {
                        return CreateTextMessage(String.Format("Could not create instance to generate example document. Failed with error '{0}'", e.Message));
                    }
                    WebInvokeAttribute invoke = od.Behaviors.Find<WebInvokeAttribute>();
                    if (GetResponseFormat(null, invoke, od) == "Json")
                    {
                        // return the example in json format
                        result = Message.CreateMessage(MessageVersion.None, null, new JsonBodyWriter(instance, body));
                        result.Properties[WebBodyFormatMessageProperty.Name] = new WebBodyFormatMessageProperty(WebContentFormat.Json);
                        WebOperationContext.Current.OutgoingResponse.ContentType = "text/plain";
                        return result;
                    }
                    else
                    {
                        try
                        {
                            return CreateExample(instance, body, isXmlSerializerType);
                        }
                        catch (Exception e)
                        {
        ...
        class JsonBodyWriter : BodyWriter
        {
            object instance;
            Type type;
            ...
            {
                DataContractJsonSerializer serializer = new DataContractJsonSerializer(this.type);
